...
import { createToken, deleteToken, getTokenKey, getTokens } from '../../support/gql'
...
describe('Get single token via API - query.admin.personalApiTokens.tokenByKey', () => {
    ...
    it('Create a token and fetch it', async function () {
        ...
        // Get Key from token
        // https://github.com/Jahia/personal-api-tokens/blob/main/src/main/java/org/jahia/modules/apitokens/core/TokenUtils.java
        const tokenKey = await getTokenKey(createdToken, client)
    ...
    it('Fetch a token using a key that does not exist anymore', async function () {
        ...
        const createdToken = await createToken('root', 'test-A', null, null, client)
        const tokenKey = await getTokenKey(createdToken, client)
        await deleteToken(tokenKey, client)
